...
expect_identical(
  result$status[1],
  0L,
  info = "First command (view) executed successfully"
)
expect_identical(
  result$status[2],
  0L,
  info = "Second command (view -H) executed successfully"
)

# Check that output is captured correctly
expect_true(
  is.character(result$stdout),
  info = "Stdout captured as character vector"
)
...
expect_identical(
  result2$status[1],
  0L,
  info = "First command (view -h) executed successfully"
)
expect_identical(
  result2$status[2],
  0L,
...
expect_error(
  BCFToolsPipeline("invalid_cmd", character(0), "view", c("-h")),
  pattern = "not a recognized bcftools command",
  info = "Error on invalid first command"
)

expect_error(
  BCFToolsPipeline("view", c("-h"), "invalid_cmd", character(0)),
  pattern = "not a recognized bcftools command",
