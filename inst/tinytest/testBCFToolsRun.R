...
out <- BCFToolsRun("version", character(),
    catchStdout = TRUE,
    catchStderr = TRUE
)
expect_equal(out$status, 0L, info = "BCFToolsRun with updated signature works correctly")
expect_true(is.character(out$command), info = "Command returned")
expect_true(length(out$stdout) > 0, info = "Stdout captured")

# Test command validation
expect_error(BCFToolsRun("nonexistent"),
...
expect_error(BCFToolsRun("index", nonexistentFile),
...
if (file.exists(vcfFile)) {
    ...
    expect_true(any(grepl("##fileformat=VCF", out$stdout)),
    ...
    expect_true(any(grepl("summary numbers", out$stdout, ignore.case = TRUE)),
        info = "Stats contains summary information"
    )

    # Test query directly using BCFToolsRun
    out <- BCFToolsRun("query", c("-f", "%CHROM\\t%POS\\t%REF\\t%ALT\\n", "-H", vcfFile))
    expect_equal(out$status, 0L, info = "BCFToolsRun query command works")
    expect_true(any(grepl("CHROM", out$stdout)),
