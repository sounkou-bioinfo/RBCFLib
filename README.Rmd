...
- Genotypes: `VariantGenotype()`, `VariantGenotypesAlleleIdx0()`, `GenotypePloidy()`, `GenotypeHomRef()`, `GenotypeHet()` …
...
RBCFLib provides direct access to VCF/BCF header metadata, including INFO, FORMAT, FILTER, ALT, CONTIG fields, and sample names, fully parsed and mapped to R types.

## Header-only API

To retrieve header metadata from any VCF/BCF file (no index or context required):

```{r header-only-api, eval=TRUE, echo=TRUE}
...
When working with VBI context, header metadata is available via the context object and can be printed in R:

```{r vbi-header-metadata-example, eval=TRUE, echo=TRUE}
vbiFile <- tempfile(fileext = ".vbi")
VBIIndex(vcfFile, vbiFile)
