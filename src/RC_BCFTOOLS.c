...
SEXP RC_bcftools_run(
  ...
  const char* cmd_str = CHAR(STRING_ELT(command, 0));
  ...
  int bc_n   = user_n + 2;            // +1 for "bcftools", +1 for command
  char *bc_argv[bc_n];
  ...
  bc_argv[1] = (char*)cmd_str;

  // shift user args into argv[2..]
  for (i = 0; i < user_n; i++) {
    bc_argv[i+2] = user_argv[i];
  }
