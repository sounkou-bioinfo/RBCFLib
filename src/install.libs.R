...
htslib_headers <- c(
    list.files("htslib-1.21/", pattern = "\\.h$", full.names = TRUE)
)
...
htslib_libs <- c(
    list.files("htslib-1.21/", pattern = paste0(".*", SHLIB_EXT), full.names = TRUE)
)
htslib_libs <- c(htslib_libs, dir("htslib-1.21/", pattern = "a$", full.names = TRUE))
