...
#ifdef HAVE_CHOLMOD
#include "pgs.RBCFLIB.c"
#endif

SEXP RC_bcftools_pgs(
                  SEXP args, 
                  SEXP capture_stdout,
                  SEXP capture_stderr, 
                  SEXP stdout_file,
...
#ifndef HAVE_CHOLMOD
  error("BCFTools was compiled without CHOLMOD support. Please install CHOLMOD and recompile.");
  return R_NilValue;
#else
  ...
  // Run the command
  status = run_pgs(nargs, user_argv);
 ...
 // Close file handles
  ...
  char full_cmd[1000] = "bcftools pgs";
  for (i = 0; i < nargs; i++) {
    strcat(full_cmd, " ");
