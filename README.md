...
#> Wrote 15 index records into /tmp/RtmpApQveM/file24cc6664fa0e0.vbi
#> Indexing  finished: 3202 samples, 15 markers, 1 chromosomes
#> VBI index created: /tmp/RtmpApQveM/file24cc6664fa0e0.vbi
#> [1] "/tmp/RtmpApQveM/file24cc6664fa0e0.vbi"

# Load index as external pointer
vbi_ptr <- VBIIndexLoad(vbiFile)

# Extract all variant ranges
ranges <- VBIExtractRanges(vbi_ptr)
print(paste("Total variants:", nrow(ranges)))
#> [1] "Total variants: "

# Query by variant index range (1-based, inclusive)
hits_by_index <- VBIQueryByIndices(vcfFile, vbi_ptr, 1, 5)
print(paste("Variants 1-5:", nrow(hits_by_index)))
#> [1] "Variants 1-5: 5"

# Query by genomic region (linear scan)
hits_region <- VBIQueryRange(vcfFile, vbi_ptr, "chr21:5030082-5030356")
print(paste("Region query hits:", nrow(hits_region)))
#> [1] "Region query hits: 9"

# Query by genomic region (cgranges - faster for complex queries)
hits_cgranges <- VBIQueryRegionCGRanges(vcfFile, vbi_ptr, "chr21:5030081-5030357")
print(paste("CGRanges query hits:", nrow(hits_cgranges)))
#> [1] "CGRanges query hits: 9"
